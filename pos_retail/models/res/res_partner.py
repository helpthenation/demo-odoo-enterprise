...
class res_partner(models.Model):
    ...
    def _compute_debit_credit_balance(self):
        for partner in self:
            ...
            partner.balance = 0
            self.env.cr.execute(
                "select sum(amount) from res_partner_credit where partner_id=%s and type='plus'" % partner.id)
            credits1 = self.env.cr.fetchall()
            self.env.cr.execute(
                "select sum(amount) from res_partner_credit where partner_id=%s and type='redeem'" % partner.id)
            debits1 = self.env.cr.fetchall()
            if credits1:
                for credit in credits1:
                    if credit[0]:
                        partner.credit += credit[0]
            if debits1:
                for debit in debits1:
                    if debit[0]:
                        partner.debit += debit[0]
            self.env.cr.execute(
                "select sum(amount_total) from account_invoice where partner_id= %s and type = 'out_refund' and state NOT IN  ('draft', 'cancel') and add_credit is True" % partner.id)
            credits2 = self.env.cr.fetchall()
            if credits2:
                for credit in credits2:
                    if credit[0]:
                        partner.credit += credit[0]
            self.env.cr.execute(
                "select sum(amount_total) from account_invoice where partner_id= %s and type = 'in_refund' and state NOT IN  ('draft', 'cancel') and add_credit is True" % partner.id)
            debits2 = self.env.cr.fetchall()
            if debits2:
                for debit in debits2:
                    if debit[0]:
                        partner.debit += debit[0]
