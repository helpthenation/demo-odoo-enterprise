...
class pos_order(models.Model):
    ...
    def create_from_ui(self, orders):
        _logger.info('->> {create_from_ui} start')
        for o in orders:
            data = o['data']
            if o['to_invoice'] and data.get('is_return', False):  # block create invoice when order is return
                o['to_invoice'] = False
        ...
        for order in orders_object:
            ...
            if order.email and order.email_invoice and invoices:
                for inv in invoices:
                    _logger.info('->> auto email send receipt to customer')
    ...
    def pos_made_return_order(self):
        """
            * create credit note for return order
        """
        Invoice = self.env['account.invoice']
        if self.is_return and self.partner_id and self.add_credit:
            _logger.info('->> pos_made_return_order')
            inv_credit_note = {
                'name': self.name,
                'origin': self.name,
            ...
            local_context = dict(self.env.context, force_company=self.company_id.id,
                                 company_id=self.company_id.id)
            if self.session_id.config_id.add_credit:
                inv_credit_note['add_credit'] = True
            invoice = Invoice.new(inv_credit_note)
            ...
            for line in self.lines:
                self.with_context(local_context)._action_create_invoice_line_return(line, new_invoice.id)
            new_invoice.with_context(local_context).sudo().compute_taxes()
            self.sudo().write({'state': 'invoiced'})
            _logger.info('credit note [%s]' % new_invoice.number)
            return new_invoice.action_invoice_open()
