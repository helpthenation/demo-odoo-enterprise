odoo.define('pos_retail.popups', function (require) {
                                ...
                                for (var i = 0; i < refund_invoices.length; i++) {
                                    ...
                                    rpc.query({ // update invoice
                                        ...
                                        open_inv.then(function (result) {
                                            var message = "<a class='so_link' target='_blank' href=" + window.location.origin + "/web#id=" + self.refund_invoice['id'] + "&view_type=form&model=account.invoice +>" + '(Click here)' + "</a>";
        ...
        renderElement: function () {
            ...
            this.$('.confirm').click(function () {
                ...
                self.gui.show_popup('alert_confirm', {
                    title: 'Requesting Backend',
                    body: 'Please waiting, order just submit to backend',
                    confirmButtonText: 'Yes',
                    cancelButtonText: 'Close',
                    confirm: function () {
                        self.pos.gui.close_popup();
    ...
    var popup_create_purchase_order = PopupWidget.extend({ // create purchase order
        ...
        create_po: function () {
            ...
            return rpc.query({
                ...
                self.pos.get_order().destroy();
                var body = "<a class='so_link' target='_blank' href='" + window.location.origin + "/web#id=" + result['id'] + "&view_type=form&model=purchase.order'" + ">";
                body += result.name + "(Click here)</a>"
                return self.gui.show_popup('alert_confirm', {
    ...
    var popup_return_pos_order_lines = PopupWidget.extend({
        ...
        show: function (options) {
            ...
            if (self.line_selected) {
                self.$el.find('tbody').html(qweb.render('pos_order_line', {
    ...
    var popup_lock_session = PopupWidget.extend({
        ...
        show: function (options) {
            var self = this;
            this._super(options);
            $('.ask_password').click(function () {
                self.pos.gui.show_popup('password', {
                    'title': _t('Password ?'),
                    confirm: function (pw) {
                        if (self.pos.config.lock_session_password == false) {
                            $('.pos-content').removeClass('oe_hidden');
                            return self.pos.gui.close_popup();
                        }
                        else if (pw !== self.pos.config.lock_session_password) {
                            self.pos.gui.show_popup('error', _t('Incorrect Password'));
                            return setTimeout(function () {
                                self.pos.gui.show_popup('popup_lock_session');
                            }, 1000);
                        } else {
                            $('.pos-content').removeClass('oe_hidden');
                            return self.pos.gui.close_popup();
