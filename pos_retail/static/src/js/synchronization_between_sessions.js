odoo.define('pos_retail.synchronization', function (require) {
    ...
    models.PosModel = models.PosModel.extend({
        ...
        syncing_sessions: function (message) {
            this.set('synch', {state: 'connecting', pending: 1});
        ...
        sync_order_removing: function (uid) {
            ...
            if (order) {
                order.syncing = true;
                var current_order = this.get_order();
                if (current_order && current_order['uid'] == uid) {
                    setTimeout(function () {
    ...
    models.Order = models.Order.extend({
        ...
        set_client: function (client) {
            ...
            if (client && client['property_product_pricelist']) {
                ...
                if ((this.pos.version.server_serie == '11.0' || this.pos.version.server_serie == '11.0+e') && pricelist && !this.is_return) { // we're don't want set price list for order return
                    this.set_pricelist(pricelist)
                }
                if ((this.pos.version.server_serie == '10.0' || this.pos.version.server_serie == '10.0+e') && pricelist && !this.is_return) { // we're don't want set price list for order return
